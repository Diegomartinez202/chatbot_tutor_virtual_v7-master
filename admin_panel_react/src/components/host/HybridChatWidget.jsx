...
const CHAT_IFRAME_SRC = "/chat?embed=1&guest=1"; // ajusta si sirve en otra ruta/origin
...
export default function HybridChatWidget({ defaultOpen = false }) {
    ...
    useEffect(() => {
        const onMsg = (ev) => {
            ...
            if (data.type === "auth:request") {
                // el iframe (chat) pide token → lo mandamos
    ...
    // Empuja token cuando el iframe carga
    function onLoad() { sendAuthTokenToIframe(); }
            ...
            {open && (
                ...
                >
                    <div
                        ...
                        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", background: "#1e293b", color: "#fff", fontSize: 14 }}
                    >
                        <strong>Tutor Virtual</strong>
                        <div style={{ display: "flex", gap: 6 }}>
                            <button type="button" onClick={() => setMinimized((m) => !m)} className="btn" style={{ padding: "4px 8px" }}>
                                {minimized ? "Maximizar" : "Minimizar"}
                            </button>
                            <button type="button" onClick={() => setOpen(false)} className="btn" style={{ padding: "4px 8px" }}>
                                Cerrar
                            </button>
                        </div>
                    </div>
