...
function HostChatBubbleImpl({
  ...
  // API de conveniencia (para llamadas vía ref desde App)
  const sendToken = useCallback((token) => {
    bubbleRef.current?.sendAuthToken?.(token);
  }, []);
  const setThemeApi = useCallback(
    (next) => {
  ...
  const setLanguageApi = useCallback((lang) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    minimize:(v) => bubbleRef.current?.minimize?.(v),
    sendAuthToken: (t) => bubbleRef.current?.sendAuthToken?.(t),
    setTheme: (t) => setThemeApi(t),
    setLanguage: (l) => setLanguageApi(l),
  }), [setThemeApi, setLanguageApi]);
    ...
    <div style={{ position: "fixed", left: 8, bottom: 8, padding: 8, zIndex: zIndex + 1 }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
          ...
          <button
            onClick={() => setThemeApi(lastPrefs.theme === "dark" ? "light" : "dark")}
            style={btn}
          >
            Toggle theme
          </button>
          <button
            onClick={() => setLanguageApi(lastPrefs.language === "es" ? "en" : "es")}
            style={btnSecondary}
          >
            Toggle lang
          </button>
...
export default forwardRef(HostChatBubbleImpl);
