...
def _get_certificados_desde_slot_o_mock(tracker: Tracker) -> List[Dict[str, str]]:
    ...
    # Si el slot existe y es lista:
    if isinstance(data, list):
        # Si está vacío, devolvemos [] => se usará el fallback estático
        if not data:
            return []

        # Normalizamos claves si hay elementos
    ...
    # Si no hay slot, usamos mock por defecto (seguro para demo)
    return [
        {
...
class ActionConsultarCertificados(Action):
    ...
    Si no se encuentran certificados (slot 'certificados' vacío),
    se usa la utter estática 'utter_certificados_carousel' como fallback.
    """
    ...
    def run(
        ...
        # >>> NUEVO: fallback "bonito" si no hay certificados <<<
        if not certificados:
            # Aquí usamos la utter estática que definiste en domain.yml
