...
class ActionAnalizarEstadoUsuario(Action):
    ...
    def run(
        ...
        # MEJORA: Extracción y normalización segura del slot emocional para evitar excepciones por tipos None
        slot_emocion = tracker.get_slot("emocion_detectada")
        ultima_emocion = str(slot_emocion).strip().lower() if slot_emocion else ""
