...
from utils.guardian_client import GuardianClient  # 👈 necesario para ActionGuardarAutosave
...
class ActionRegistrarEncuestaGuardian(Action):
    def name(self) -> str:
        # 👀 Nombre único, para no chocar con action_registrar_encuesta de encuesta normal
...
class ActionGuardarAutosave(Action):
    ...
    def run(
        ...
        if ok:
            ...
            dispatcher.utter_message(
                text="No se pudo guardar el autosave ❌ (no bloqueante)"
            )
        return []
